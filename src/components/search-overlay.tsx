...
function getAllItems(bookData: LibraryData, comicData: LibraryData): SearchResult[] {
  ...
  for (const items of Object.values(bookData)) {
    if (items) items.forEach((item) => results.push({ section: "Books", item, searchTitle: item.title.toLowerCase(), searchAuthor: item.author.toLowerCase() }));
  }
  for (const items of Object.values(comicData)) {
    if (items) items.forEach((item) => results.push({ section: "Comics", item, searchTitle: item.title.toLowerCase(), searchAuthor: item.author.toLowerCase() }));
  }
...
export function SearchOverlay({ open, onClose, bookData, comicData }: SearchOverlayProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return allItems.filter(
      (r) =>
        r.searchTitle.includes(q) ||
        r.searchAuthor.includes(q)
    );
  }, [query, allItems]);
