...
function getAllItems(bookData: LibraryData, comicData: LibraryData): SearchResult[] {
  ...
  for (const items of Object.values(bookData)) {
    if (items) items.forEach((item) => results.push({
      section: "Books",
      item,
      searchString: `${item.title.toLowerCase()}\0${item.author.toLowerCase()}`
    }));
  }
...
export function SearchOverlay({ open, onClose, bookData, comicData }: SearchOverlayProps) {
  ...
  const filtered = useMemo(() => {
    ...
    // OPTIMIZATION: Filter using the pre-computed lowercased searchString to avoid
    // repeatedly calling .toLowerCase() on item properties during every render.
