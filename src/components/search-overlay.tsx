...
export function SearchOverlay({ open, onClose, bookData, comicData }: SearchOverlayProps) {
  ...
  // ⚡ Bolt Optimization: Pre-compute lowercased concatenated strings for search
  // Impact: Reduces O(N) object property access and string allocation per keystroke to a fast O(N) array lookup.
  const searchStrings = useMemo(() => {
    return allItems.map((r) => `${r.item.title.toLowerCase()}\0${r.item.author.toLowerCase()}`);
  }, [allItems]);
