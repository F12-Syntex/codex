...
export function SearchOverlay({ open, onClose, bookData, comicData }: SearchOverlayProps) {
  ...
  // Pre-compute lowercased search strings to prevent recreating strings inside the tight .filter() loop
  // We use a null byte \0 to safely concatenate fields without causing false positive cross-field matches
  const searchableItems = useMemo(() => {
    return allItems.map((r) => ({
      ...r,
      searchString: `${r.item.title}\0${r.item.author}`.toLowerCase()
    }));
  }, [allItems]);
  ...
  const filtered = useMemo(() => {
    ...
    return searchableItems.filter((r) => r.searchString.includes(q));
