...
function getAllItems(bookData: LibraryData, comicData: LibraryData): SearchResult[] {
  ...
  for (const items of Object.values(bookData)) {
    if (items) items.forEach((item) => results.push({ section: "Books", item, searchString: `${item.title.toLowerCase()}\0${item.author.toLowerCase()}` }));
  }
  for (const items of Object.values(comicData)) {
    if (items) items.forEach((item) => results.push({ section: "Comics", item, searchString: `${item.title.toLowerCase()}\0${item.author.toLowerCase()}` }));
  }
