...
interface SavedQuoteWithSearch extends SavedQuote {
...
export function QuotesView({ filePath, bookTitle }: QuotesViewProps) {
  ...
  // ⚡ Bolt Optimization: Pre-compute lowercased concatenated search strings to avoid O(N)
  // string transformations on every keystroke. Using \0 prevents cross-field false matches.
  const quotesWithSearch = useMemo<SavedQuoteWithSearch[]>(() => {
    return quotes.map((q) => ({
      ...q,
      _searchString: `${q.text}\0${q.speaker}\0${q.chapterTitle}\0${q.bookTitle}`.toLowerCase(),
    }));
  }, [quotes]);

  const filtered = useMemo(() => {
    const trimmedSearch = search.trim();
    const searchLower = trimmedSearch ? trimmedSearch.toLowerCase() : "";

    return quotesWithSearch.filter((q) => {
      if (filterKind !== "all" && q.kind !== filterKind) return false;
      if (filterBook !== "all" && q.bookTitle !== filterBook) return false;
      if (searchLower && q._searchString) {
        if (!q._searchString.includes(searchLower)) return false;
      }
      return true;
    });
  }, [quotesWithSearch, filterKind, filterBook, search]);
