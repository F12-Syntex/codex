...
export function QuotesView({ filePath, bookTitle }: QuotesViewProps) {
  ...
  const searchStrings = useMemo(() => {
    return quotes.map((q) => `${q.text}\0${q.speaker}\0${q.chapterTitle}\0${q.bookTitle}`.toLowerCase());
  }, [quotes]);
  ...
  const filtered = useMemo(() => {
    return quotes.filter((q, i) => {
      ...
      if (search.trim()) {
        ...
        if (!searchStrings[i].includes(s)) return false;
      }
      return true;
    });
  }, [quotes, searchStrings, filterKind, filterBook, search]);
