...
export function QuotesView({ filePath, bookTitle }: QuotesViewProps) {
  ...
  // Isolate expensive pre-computation of searchable strings from filtering
  ...
  const filtered = useMemo(() => {
    ...
    return quotes.filter((q, i) => {
      ...
      if (searchNormalized) {
        if (!searchStrings[i].includes(searchNormalized)) return false;
      }
      return true;
    });
  }, [quotes, filterKind, filterBook, search, searchStrings]);
