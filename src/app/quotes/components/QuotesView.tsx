...
export function QuotesView({ filePath, bookTitle }: QuotesViewProps) {
  ...
  // Pre-compute lowercased searchable strings on the quote objects during initialization
  // to avoid recalculating the same .toLowerCase() strings on every keystroke during filtering.
  // Using \0 separator ensures boundaries are respected.
  const searchableQuotes = useMemo(() => {
    return quotes.map((q) => ({
      ...q,
  ...
  const filtered = useMemo(() => {
    ...
    return searchableQuotes.filter((q) => {
      if (filterKind !== "all" && q.kind !== filterKind) return false;
      if (filterBook !== "all" && q.bookTitle !== filterBook) return false;
      if (s) {
        if (!q._searchString.includes(s)) return false;
      }
      return true;
    });
  }, [searchableQuotes, filterKind, filterBook, search]);
