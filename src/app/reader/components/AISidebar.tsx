...
export function AISidebar({
  ...
  const allDone = enrichEnabled && chaptersToEnrichCount === 0 && alreadyEnriched > 0 && !isRunningAll;
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto ${disabled ? "opacity-50 pointer-events-none select-none" : ""}`}>
        ...
        <div className="space-y-0.5 px-1.5">
          {/* Enrich Chapters */}
          <div className={`rounded-lg transition-colors ${enrichEnabled && !disabled ? "bg-[var(--accent-brand)]/5" : ""}`}>
            {/* Main row */}
            ...
            {enrichEnabled && !disabled && (
              <div className={`mx-3 mb-2.5 rounded-lg px-3 py-2.5 ${theme.subtle}`}>
                {/* Progress bar — bulk enrichment */}
                {isRunningAll && (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-[11px] tabular-nums ${theme.muted}`}>
                        {enrichAllProgress.current + 1} of {enrichAllProgress.total}
                      </span>
                    </div>
                  </div>
                )}

                {/* Single chapter indicator */}
                {!isRunningAll && enrichingChapter !== null && (
                  <div className="flex items-center gap-1.5">
                    <Loader2 className="h-3 w-3 animate-spin text-[var(--accent-brand)]" strokeWidth={2} />
                    <span className={`text-[11px] ${theme.muted}`}>
                ...
                {enrichingChapter === null && (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <span className={`text-[11px] ${theme.muted}`}>
                        {alreadyEnriched > 0
                          ? `${alreadyEnriched} renamed` + (chaptersToEnrichCount > 0 ? ` · ${chaptersToEnrichCount} remaining` : "")
                          : `${chaptersToEnrichCount} chapter${chaptersToEnrichCount !== 1 ? "s" : ""} to rename`
                        }
                      </span>
                      {allDone && (
                        <span className="text-[11px] text-[var(--accent-brand)]">All done</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      {chaptersToEnrichCount > 0 && (
                        <button
                          onClick={onEnrichAll}
                          className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-1.5 text-[11px] font-medium transition-colors ${theme.text}`}
                          style={{ background: "var(--accent-brand-dim)" }}
                        >
                          <Zap className="h-3 w-3 text-[var(--accent-brand)]" strokeWidth={1.5} />
                          Enrich All
                        </button>
                      )}
                      {alreadyEnriched > 0 && (
                        <button
                          ...
                          className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] transition-colors ${theme.btn}`}
                        >
            ...
            {/* Idle hint when disabled */}
            {!enrichEnabled && chaptersToEnrichCount > 0 && enrichingChapter === null && (
              <div className={`px-3 pb-2.5 pl-[38px] text-[11px] ${theme.muted}`}>
          ...
          <SettingRow
            ...
            label="Smart Summary"
            description="Generate a recap of previous chapters when you resume"
            theme={theme}
        ...
        <div className="space-y-0.5 px-1.5">
          ...
          <SettingRow
            ...
            description="Enhance typography and layout for a polished reading feel"
            theme={theme}
