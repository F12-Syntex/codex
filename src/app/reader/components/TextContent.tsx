 ...
 *
 * Layout:
 *   outerRef  — full-height container with padding
 *   ├─ clipper — overflow:hidden, exactly pageWidth × contentHeight
 *   │  └─ slider — position:absolute, NO explicit width, column-width layout
 *   │              transform: translateX(-page * stride) for navigation
 *   └─ (inline styles for images)
 *
 * The slider has no width constraint so the browser auto-sizes it to hold
 * all the columns generated by column-width + column-fill:auto + fixed height.
 * We read slider.scrollWidth to learn how wide the full content is, then
 * divide by stride (pageWidth + columnGap) to get the page count.
 */
...
export function TextContent({
  ...
  // Each column is half the page minus half the gap
  const colWidth = Math.max(1, Math.floor((pageWidth - columnGap) / 2));

  // Stride = distance between the start of one page and the start of the next
  // in a continuous column stream: pageWidth (2 cols + 1 gap) + 1 inter-page gap
  ...
  const measure = useCallback(() => {
    ...
    // Let the browser lay out columns, then measure total width
  ...
  // Re-measure when content or typography changes
  useEffect(() => {
    measure();
  ...
  // Find first real text paragraph (not empty, not image, not heading-like)
