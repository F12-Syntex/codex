...
function stripHtmlForTTS(html: string[]): string[] {
  return html.map(h => {
    // Drop dialogue-speaker spans only when followed by a quote character, so TTS reads
    // only the quoted speech and not "Sirius, \"As you know...\"".
    // Spans NOT followed by a quote (e.g. a name mid-narration) keep their text.
    let text = h.replace(
      /<span\s+class="ai-fmt-dialogue-[^"]*">([^<]*)<\/span>(\s*,?\s*)(?=["""'''])/g,
      "",
    );
