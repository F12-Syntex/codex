          ...
          >
            {selectedEntry ? (
              <EntryPage
                ...
                computedNameMap={computedNameMap}
                onNavigate={navigateTo}
                onBack={goBack}
                onMerge={handleMerge}
                canGoBack
...
function EntryPage({
  entry, allEntries, computedNameMap, resolvedNames, onNavigate, onBack, onMerge, canGoBack, chapterLabels,
}: {
  entry: WikiEntry;
  allEntries: EntryListItem[];
  computedNameMap: { name: string; lower: string; id: string; type: WikiEntryType }[];
  ...
  return (
    <div className="mx-auto w-full max-w-[900px] px-4 py-6 sm:px-6">
      ...
      <p className="mb-6 text-sm leading-relaxed text-white/60">
        <LinkedText text={entry.shortDescription} computedNameMap={computedNameMap} currentEntryId={entry.id} onNavigate={onNavigate} />
      </p>
          ...
          {entry.description && (
            <ArticleSection title="Overview">
              <p className="text-sm leading-relaxed text-white/55 whitespace-pre-wrap">
                <LinkedText text={entry.description} computedNameMap={computedNameMap} currentEntryId={entry.id} onNavigate={onNavigate} />
              </p>
            </ArticleSection>
          )}
                ...
                {items.map((item, i) => (
                  <div key={i} className="flex gap-3 rounded-lg bg-[var(--bg-surface)] px-3 py-2">
                    ...
                    <p className="flex-1 text-xs leading-relaxed text-white/55">
                      <LinkedText text={item.content} computedNameMap={computedNameMap} currentEntryId={entry.id} onNavigate={onNavigate} />
...
function LinkedText({
  ...
  const segments = useMemo(() => {
    if (!text || computedNameMap.length === 0) return [{ text, entityId: null as string | null }];
    ...
    for (const entry of computedNameMap) {
    ...
    return result;
  }, [text, computedNameMap, currentEntryId]);
