...
## 2024-05-18 - Schwartzian Transform for Case-Insensitive String Sorting
**Learning:** Using `Intl.Collator` for sorting is significantly slower in V8/Node.js than simple `.toLowerCase()` string comparisons, and calling `.toLowerCase()` in a `.sort()` block results in $O(N \log N)$ transformations. Combining a Schwartzian transform (decorate-sort-undecorate) changes this into an $O(N)$ string mapping, significantly boosting large array sorting efficiency. Moreover, a chained `.map().sort().map()` eliminates the need to explicitly clone the original array (`[...array]`), because `.map` naturally returns a fresh reference that can be safely mutated by `.sort()`.
**Action:** Replace `const sorted = [...items].sort((a,b) => a.prop.toLowerCase() < b.prop.toLowerCase() ? -1 : 1)` with `const sorted = items.map(item => ({ item, val: item.prop.toLowerCase() })).sort((a,b) => a.val < b.val ? -1 : 1).map(x => x.item)` for expensive property string transformations to reduce allocations and computations.
