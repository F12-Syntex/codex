...
## 2026-05-17 - Optimize Search Filtering String Comparisons
**Learning:** Pre-computing concatenated lowercased strings for full-text search across multiple properties and isolating it from the user's keystroke  significantly reduces garbage collection and redundant CPU work, especially on larger libraries. It avoids calling `.toLowerCase()` repeatedly for every property of every item during the filter loop, which was a hotspot in `search-overlay.tsx`. Using a null byte (`\0`) to separate fields guarantees safety against false positive cross-field boundary matches without complex RegEx overhead.
**Action:** Always pre-compute composite lowercased searchable strings on arrays of objects mapped for UI searching, memoizing it separately from the search query filter state. Do not call `.toLowerCase()` or string manipulation primitives in tight filtering or rendering loops.
## 2025-02-12 - Optimize Search Filtering String Comparisons
**Learning:** Pre-computing concatenated lowercased strings for full-text search across multiple properties and isolating it from the user's keystroke `useMemo` significantly reduces garbage collection and redundant CPU work, especially on larger libraries. It avoids calling `.toLowerCase()` repeatedly for every property of every item during the filter loop, which was a hotspot in `search-overlay.tsx`. Using a null byte (`\0`) to separate fields guarantees safety against false positive cross-field boundary matches without complex RegEx overhead.
**Action:** Always pre-compute composite lowercased searchable strings on arrays of objects mapped for UI searching, memoizing it separately from the search query filter state. Do not call `.toLowerCase()` or string manipulation primitives in tight filtering or rendering loops.
