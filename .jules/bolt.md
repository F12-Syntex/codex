...
## $(date +%Y-%m-%d) - Pre-computing lowercase search strings in static arrays
**Learning:** Performing `toLowerCase()` operations on item properties during array `filter` loops bound to React `useMemo` hooks is incredibly expensive for large sets and will block the main thread. Specifically, executing `.toLowerCase()` twice per item in a filter block over thousands of objects can take hundreds of milliseconds, significantly degrading typing responsiveness in search components.
**Action:** When filtering static or infrequently-changing object arrays in React components using string matching, always map the objects beforehand during initialization/fetch to pre-compute and store their `.toLowerCase()` search representations (separated by a non-printable character like `\0`). Then, filter against that single pre-computed string property. This reduces complexity and entirely eliminates N allocations per keystroke, achieving near-instant filtering.
