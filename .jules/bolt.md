...
## 2025-06-05 - Search String Pre-computation
**Learning:** React Memory Performance Rule: When pre-computing search strings for an array of complex objects, avoiding `.toLowerCase()` strings allocation inside a filter `map()` function over the original objects causes redundant array object allocations. Create a parallel array of primitive search strings (e.g., `const searchStrings = items.map(i => i.val.toLowerCase())`) and filter the original array using the loop index. Also, when concatenating multiple fields for a pre-computed search string (e.g., title and author), use a non-printable separator like a null byte (`\0`) between fields to prevent search queries from erroneously matching across the boundaries of the properties.
**Action:** Extract pre-computation of primitive searchable strings out of `filter()` callback hooks, keeping them in a separate `useMemo` and looking them up via loop indexes to prevent slow filtering renders.
