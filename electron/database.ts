...
export function mergeWikiEntries(filePath: string, sourceId: string, targetId: string): void {
  const merge = db.transaction(() => {
    ...
    // ⚡ Bolt: Extracted db.prepare outside the loop to eliminate redundant parsing/compilation overhead
...
export function unmergeWikiEntries(filePath: string, mergeLogId: number): void {
  const unmerge = db.transaction(() => {
    ...
    for (const d of snapshot.details) {
      ...
      updateDetails.run(e.id, filePath, log.target_id, d.chapter_index, d.category, d.content);
    }
...
export function mergeArcs(filePath: string, sourceArcIds: string[], targetArcId: string): void {
  const merge = db.transaction(() => {
    ...
    // ⚡ Bolt: Extracted db.prepare calls outside the loop to eliminate redundant parsing/compilation overhead
    const checkSourceArc = db.prepare("SELECT id FROM wiki_arcs WHERE file_path = ? AND id = ?");
    ...
    for (const sourceId of sourceArcIds) {
      ...
      const source = checkSourceArc.get(filePath, sourceId);
      if (!source) continue;
